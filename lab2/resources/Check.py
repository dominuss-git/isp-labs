...
def Check(args):
  ...
  index = path1.find(".")
  if index == -1:
    print(f"Unknown extension: {path1}")
    return
  extension1 = path1[index:]

  index = path2.find(".")
  if index == -1:
    print(f"Unknown extension: {path2}")
    return
  extension2 = path2[index:]

  print(extension1, extension2)
  


def CreateSerializator(extension):
  pass

def CreateDeserializator(extension):
  pass
  # print(args)
...
#   try:
#     obj = imprt.SourceFileLoader("__main__", path_to_obj).load_module()
#   except:
